...
class DirectoryLayout:
    def __init__(self, root_dir:Path, keyval_separator="="):
        if keyval_separator in NOT_ALLOWED_SEPARATORS:
            msg = f"'{keyval_separator}' character is not allowed as separator. Select another one not included in {NOT_ALLOWED_SEPARATORS}"
    ...
    def build_index_table(self, entries:FileEntries, index_fields:str|list[str]|None=None, add_filepath:bool=False):
        if add_filepath:
            record_generator = map(lambda entry: entry[1] | {"filepath": str(self.get_fullpath(entry[0]))}, entries)
        else:
            record_generator = map(lambda record: record[1], entries)
...
class FlatLayout(DirectoryLayout):
    ...
    def _parse_entry(self, filepath: Path):
        ...
        parsed_components = {field:value for field, value in zip(self.fields, matches)}
...
class HierarchicalLayout(DirectoryLayout):
    ...
    """
    def __init__(self, root_dir:Path, fields:list[str], segment_separator:str=os.path.sep) -> None:
...
class FlatLayoutWithKey(DirectoryLayout):
    def __init__(self, root_dir:Path, keyval_separator:str="=", field_separator:str="$"):
        ...
        self.field_separator = field_separator
        self._discover_pattern = "*.*"
        self._pattern = re.compile(rf"([^${keyval_separator}]+){keyval_separator}([^${field_separator}\.]+)")
...
class HierarchicalLayoutWithKey(DirectoryLayout):
    def __init__(self, root_dir:Path, keyval_separator:str="=", segment_separator:str="$"):
        super().__init__(root_dir, keyval_separator)
        self.segment_separator = segment_separator
        self._pattern = re.compile(rf"([^${keyval_separator}]+){keyval_separator}([^${segment_separator}\.]+)")
